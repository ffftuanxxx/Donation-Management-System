...
<?php  
   ...
   $offset = ($p-1)*$pagesize;  
   //查询本页现实的数据 
   $query_sql = "select * from comment where b_id = $id order by name desc limit $offset,$pagesize ";  
   # echo $query_sql;  
   ...
   while($res = mysqli_fetch_array($result)){
       $idid = $res["c_id"];
       $count_result = mysqli_query($con,"select count(l_id) as count from likes where c_id = '$idid' and b_id = '$id'");  
       ...
       <span style='float:right'><a href = 'like.php?id=".$res['c_id']."&b_id=$id&type=$type'><input type='submit' class='button1' value='like'>",$count_array['count'],"</input></a></span> 
   ...
   //分页代码  
   //计算留言总数  
   $count_result = mysqli_query($con,"select count(*) as count from comment where b_id = $id");  
   $count_array = mysqli_fetch_array($count_result);  
   ...
   if($pagenum>1){  
       for($i = 1;$i<=$pagenum;$i++){  
           if($i == $p){  
               ...
               echo '<a href="details_detail.php?p=',$i,'&b_id=',$id,'&type=',$type,'">',"<div style='width:25px;display: inline-block;margin-right: 10px;background:#FF9D6F'>",$i,'</div>','</a>';
           }
       }
