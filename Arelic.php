...
echo "<br>";
$sql = "select * from relic ORDER BY `relic`.`b_id` ASC";
echo "<div class='c1'>";
$result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result) > 0) {
    // output data of each row
    echo "<br>";
    echo "<h2 align='center'>All relic </h2>";
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        for ($f = 0; $f < $fields; $f++) {
            if ($f == $fields - 7) {
                ...
                if ($f == $fields - 3) {
                    if ($row[$f] == 0) {
                        echo "<td><a href='enableThings.php?b_id=$row[0]&table=5'> enable it </a></td>";
                    } else if ($row[$f] == 1) {
                        echo "<td><a href='disableThings.php?b_id=$row[0]&table=5'style='color:red'> disable it </a></td>";
                    }
                } else {
                    echo "<td>" . $row[$f] . "</td>";
                }
        ...
        echo "<td><a href='deleteThings.php?b_id=$row[0]&table=5'> Delete it </td>";
