...
echo "<br>";
//animal--------------------------------------------------------------------------------------------------------------------
$sql = "select * from animal ORDER BY `animal`.`b_id` ASC";
...
if (mysqli_num_rows($result) > 0) {
    ...
    echo "<h2 align='center'>All animal </h2>";
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        for ($f = 0; $f < $fields; $f++) {
            if ($f == $fields - 7) {
                ...
                if ($f == $fields - 3) {
                    if ($row[$f] == 0) {
                        echo "<td><a href='enableThings.php?b_id=$row[0]&table=1'> enable it </a></td>";
                    } else if ($row[$f] == 1) {
                        echo "<td><a href='disableThings.php?b_id=$row[0]&table=1'style='color:red'> disable it </a></td>";
                    }
                } else {
                    echo "<td>" . $row[$f] . "</td>";
                }
        ...
        echo "<td><a href='deleteThings.php?b_id=$row[0]&table=1'> Delete it </td>";
